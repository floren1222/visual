...
public final class BlockESP extends Module {
    ...
    // Убрана настройка onlyVisible - теперь всегда показываем только видимые блоки
    ...
    public void render(EventRender3D eventRender3D) {
        ...
        for (BlockPos pos : blockPos) {
            ...
            if (itemSelectSetting.contains(block)) {
                // Всегда проверяем видимость блока - показываем только видимые блоки
    ...
    private boolean isBlockVisible(BlockPos blockPos) {
        ...
        // Создаем контекст для ray tracing
        ...
        // Выполняем ray tracing
        var result = mc.world.raycast(context);
        
        // Если ray tracing попал в целевой блок, значит он видим
